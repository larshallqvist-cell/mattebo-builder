  ...
  return <div className="bg-card rounded-lg border border-border overflow-hidden h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <Accordion type="single" collapsible className="w-full">
          {resources.map(category => <AccordionItem key={category.id} value={category.id} className="accordion-chapter border-b-2 border-chalk-yellow/40">
              ...
              <AccordionContent className="bg-muted/30">
                ...
                const linkContent = <a key={index} href={href} target={isExternal ? "_blank" : undefined} rel={isExternal ? "noopener noreferrer" : undefined} className="flex items-center gap-2 py-2 px-3 rounded-md bg-transparent hover:bg-accent/10 transition-all duration-300 ease-out group font-body font-normal border-b border-chalk-yellow/25">
