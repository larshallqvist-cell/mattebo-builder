          ...
          {resources.map((category) => (
            <AccordionItem key={category.id} value={category.id} className="accordion-chapter">
              ...
              <AccordionContent className="bg-muted/30">
                <div className="px-2 py-1 grid grid-cols-1 sm:grid-cols-2 gap-x-2 gap-y-0.5 auto-rows-min grid-flow-row-dense">
                  {category.links.map((link, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-xs leading-snug text-[#d7e7fe]">{link.title}</span>
                      </a>
                    );
                  })}
                </div>
