...
const PostItNote = ({
  ...
  const parseContent = (text: string) => {
    ...
    const flushBulletList = () => {
      if (bulletItems.length > 0) {
        elements.push(<ul key={`ul-${elements.length}`} className="list-disc list-inside space-y-0.5 my-1 font-body font-normal">
            {bulletItems.map((item, i) => <li key={i} className="leading-snug text-lg">{parseInline(item)}</li>)}
          </ul>);
        bulletItems = [];
      }
    };
    const flushNumberedList = () => {
      if (numberedItems.length > 0) {
        elements.push(<ol key={`ol-${elements.length}`} className="list-decimal list-inside space-y-0.5 my-1 font-body font-normal">
            {numberedItems.map((item, i) => <li key={i} className="text-xl leading-snug">{parseInline(item)}</li>)}
          </ol>);
        numberedItems = [];
      }
    };
      ...
      while ((match = pattern.exec(text)) !== null) {
        ...
        if (token.startsWith('**') && token.endsWith('**')) {
          ...
          if (linkMatch) {
            ...
            result.push(<a key={`a-${keyIndex++}`} href={href} className="text-primary-foreground/80 underline hover:text-primary-foreground" target="_blank" rel="noopener noreferrer">
                {linkMatch[1]}
              </a>);
          }
        }
    ...
    lines.forEach((line, i) => {
      ...
      if (trimmed.startsWith('### ')) {
        flushBulletList();
        flushNumberedList();
        elements.push(<h6 key={`h6-${i}`} className="text-xl font-semibold mt-2 mb-0.5 font-body leading-snug">
            {parseInline(trimmed.slice(4))}
          </h6>);
      } else if (trimmed.startsWith('## ')) {
        flushBulletList();
        flushNumberedList();
        elements.push(<h5 key={`h5-${i}`} className="text-2xl font-semibold mt-2 mb-0.5 font-body leading-snug">
  ...
  return <div className="post-it-straight max-w-full">
      ...
      <div className="space-y-0">
        {loading ? <p className="text-base text-muted-foreground">Laddar...</p> : content ? parseContent(content) : <p className="text-base text-muted-foreground">Ingen beskrivning tillgänglig</p>}
      </div>
    </div>;
};
